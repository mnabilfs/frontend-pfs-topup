...
const TopupPage = () => { 
  ...
  // --- useEffect untuk memuat data dan listener 'storage' ---
  useEffect(() => {
    loadData();
  ...
  // --- LOGIKA FUNGSI (handleLookup, dll) ---
  ...
  // --- FUNGSI HANDLEBUYCLICK YANG SUDAH LENGKAP & BERSIH ---
  const handleBuyClick = async () => {
    ...
    if (gameDetails && gameDetails.name.toLowerCase() === 'mobile legends') {
      ...
      // --- LOGIKA VALIDASI ASLI ANDA YANG DIKEMBALIKAN ---
      if (userId.length < 8) {
        setFormError(false);
      ...
      // --- AKHIR DARI LOGIKA VALIDASI ASLI ---
    ...
    // Proses Fetch (sama seperti sebelumnya)
    try {
  ...
  // --- TAMPILAN / JSX (Biarkan Sama) ---
  if (isLoading || !gameDetails) {
    return <div className="bg-gray-800 min-h-screen text-white text-center p-20">Loading...</div>
  }
    ...
    <div className="w-full bg-gray-800">
                  ...
                  {gameDetails.name.toLowerCase() === 'mobile legends' ? (
                    <div className="grid items-center grid-cols-3 gap-2">
                      <div className="col-span-2">
                        <input id="idplayer" type="number" placeholder="Masukan User ID" value={userId}
                          onChange={(e) => { const value = e.target.value.slice(0, 9); setUserId(value); }}
                          required className="w-full h-8 p-3 text-xs tracking-wide text-center placeholder-gray-400 bg-white rounded-lg md:h-10 focus:outline-none focus:ring-1 focus:border-purple-200"
                        />
                      </div>
                      <div className="col-span-1">
                        <input id="idserverplayer" type="number" placeholder="Zone ID" value={zoneId}
                          onChange={(e) => { const value = e.target.value.slice(0, 4); setZoneId(value); }}
                          required className="w-full h-8 p-3 text-xs tracking-wide text-center placeholder-gray-400 bg-white rounded-lg md:h-10 focus:outline-none focus:ring-1 focus:border-purple-200"
                        />
                      </div>
                    </div>
                  ) : (
                    <input id="idplayer" type="text" placeholder="Masukan User ID" value={userId}
                      onChange={(e) => setUserId(e.target.value)}
                      required className="w-full h-8 p-3 text-xs tracking-wide text-center placeholder-gray-400 bg-white rounded-lg md:h-10 focus:outline-none focus:ring-1 focus:border-purple-200"
                    />
                  )}
                  {formError && ( <p className="text-xs text-red-500">*ID Player harus diisi.</p> )}
                  {userIdError && ( <p className="text-xs text-red-500">{userIdError}</p> )}
                  {zoneIdError && ( <p className="text-xs text-red-500">{zoneIdError}</p> )}
                  {nicknameError ? ( <p className="text-xs text-red-500">{nicknameError}</p> ) 
                   : ( nickname && ( <p className="text-xs text-white"> Nickname: <span className="font-semibold">{nickname}</span> </p> ) 
                  )}
                </form>
              </Card>
            </div>
              ...
              <Card className="!bg-purple-900 !border-purple-900 w-full">
                ...
                >
                  <div>
                    <div className="block mb-4">
                      <Label htmlFor="inputWa" className="text-xs text-white">
                        Opsional: Jika ingin mendapatkan bukti...
                      </Label>
                    </div>
                    <input
                      id="inputWa" // Ganti id agar sesuai dengan label htmlFor
                      type="tel" 
                      placeholder="Harap Masukan Nomer Whatsapp"
                      value={waNumber}
                      onChange={(e) => { const value = e.target.value.slice(0, 13); setWaNumber(value); }}
                  ...
                  <div className="flex items-center gap-2 md:gap-1">
                    <Checkbox id="AccWa" className="!bg-purple-900 !border-1 !border-white focus:ring-2 hover:!bg-gray-600 w-6 h-4.5 md:w-4 md:h-4" />
                    <p className="text-xs text-white"> Ya, Saya ingin menerima berita...</p>
                  </div>
                  <Button
                    type="submit"
                    className="self-end cursor-pointer h-9 w-35 rounded-3xl text-md"
                    disabled={nicknameError !== "" && gameDetails && gameDetails.name.toLowerCase() === 'mobile legends'}
                  >
                    Beli
                  </Button>
                </form>
      ...
      {openModal && !nicknameError && (
        <ModalDetailPesanan
          ...
          data={{ selectedTopup, userId, zoneId, waNumber }}
          userId={userId}
          zoneId={zoneId}
          nickname={nickname}
          snapToken={snapToken}
